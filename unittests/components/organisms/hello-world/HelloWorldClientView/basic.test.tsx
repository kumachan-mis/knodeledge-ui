...
test('should show message from Hello World API', async () => {
  ...
  await waitFor(() => {
    expect(screen.queryByText(`Hello, ${USER.name}#0!`)).toBeInTheDocument();
  });
  ...
  expect(global.fetch).toHaveBeenNthCalledWith(
    ...
    expect.objectContaining({ method: 'POST', body: JSON.stringify({ name: `${USER.name}#0` }) }),
  );
});

test('should update message when COUNT UP button click', async () => {
  global.fetch = (global.fetch as jest.Mock)
    .mockReturnValueOnce(Promise.resolve(createOkResponse({ message: `Hello, ${USER.name}#0!` })))
    .mockReturnValueOnce(Promise.resolve(createOkResponse({ message: `Hello, ${USER.name}#1!` })));
  ...
  await waitFor(() => {
    expect(screen.queryByText(`Hello, ${USER.name}#1!`)).toBeInTheDocument();
  });
  ...
  expect(global.fetch).toHaveBeenNthCalledWith(
    2,
    `${process.env.NEXT_PUBLIC_APP_URL}/api/hello-world`,
    expect.objectContaining({ method: 'POST', body: JSON.stringify({ name: `${USER.name}#1` }) }),
  );
});
